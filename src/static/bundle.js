...
(function ($) {
  ...
  switch (id) {
    ...
    case "landing":
      var $bar = $("#bar"),
        $items = $("#items");
      if (!browser.mobile && !$bar.hasClass("docked")) {
        var offset = 50;
        $window.on("scroll", function () {
          if (!$bar.hasClass("docked")) {
            if (
              $window.scrollTop() > $items.offset().top - offset &&
              !breakpoints.active("<=narrow")
            ) {
              $bar.addClass("docked");
            ...
            if (
              ...
              window.setTimeout(function () {
                $bar.removeClass("docked").hide().fadeIn("fast");
              }, 250);
            }
          }
        });
      ...
      if (browser.mobile)
        $items.children("article").each(function () {
          var t = $(this);
          t.find("a.image, a.demo").attr(
            "href",
            "/uploads/demos/" + t.data("name")
          );
        });
      (function () {
        var $window = $(window),
        ...
        $.fn.onVisible = function (fn, p) {
          elements.push({ o: $(this), fn: fn, pad: p ? p : pad, state: false });
        };
      })();
      if (browser.os == "wp")
        $(".item .image").each(function () {
          var $t = $(this);
          ($p = $t.children(".placeholder")), (src = $p.data("src"));
          $p.attr("src", src);
        ...
        $(".item .image").each(function () {
          var $t = $(this),
            ...
            src = $p.data("src"),
            delay = browser.name == "chrome" ? 30 : 0;
          $t.onVisible(function () {
            $p.remove();
            $i.appendTo($t).attr("src", src);
            window.setTimeout(function () {
              if ($i.get(0).complete) $t.addClass("is-cached");
              else
                $i.on("load", function () {
      ...
      $window.on("load", function () {
        ...
        $window.on("resize", function () {
          if ($selector.is(":visible")) {
            ...
            if ($window.width() >= 1200) $selector.show();
          }
        });
        ...
        $selector_li.each(function () {
          ...
          t.click(function () {
            if (isLocked || t.hasClass("active")) return false;
            isLocked = true;
            $selector_li.removeClass("active");
            t.addClass("active");
            $iframeWrapper.addClass("loading");
            window.setTimeout(function () {
              if (framed) $iframeWrapper.addClass("framed");
              else $iframeWrapper.removeClass("framed");
              $iframeWrapper
                .css("left", left)
                .css("top", top)
                .css("width", width)
  ...
  var _dialog = $(
      '<div id="dialog">' +
        ...
        '<div class="actions">' +
        '<ul class="share"></ul>' +
        '<a href="http://twitter.com/ajlkn" class="offsite button icon icon-twitter twitter on">Follow @ajlkn</a>' +
        "</div>" +
        '<form method="post" action="#" class="signup">' +
        '<input type="text" name="email" id="email" placeholder="Subscribe for email updates!" value="" />' +
        ...
        '<p>Check out <a href="https://pixelarity.com" class="offsite"><strong>Pixelarity</strong></a>, my latest and greatest side project that offers attribution-free ' +
        "usage of <strong>all</strong> of my templates, exclusive new templates, and support (from me) for just <strong>$19</strong>.</p>" +
        '<a href="https://pixelarity.com" class="offsite button alt">Visit Pixelarity</a>' +
        "</section>" +
    ...
    _dialog_share = _dialog.find(".share"),
    _dialog_timeout,
  ...
  _dialog_modal.click(function (e) {
    e.stopPropagation();
  });
  _dialog_modal_closer.click(function (e) {
    _dialog.h5u_close();
    return false;
  });
    ...
    _dialog.fadeTo("fast", 1, function () {
      if (_dialog_new) {
        ...
        if (
          ...
          typeof twttr.widgets !== "undefined"
        )
          twttr.widgets.load();
      }
      _dialog_locked = false;
  ...
  _dialog.h5u_close = function () {
    if (!_dialog.is(":visible")) return false;
    window.clearTimeout(_dialog_timeout);
    _dialog_locked = true;
    _dialog.fadeTo("fast", 0, function () {
      _dialog.hide();
      _dialog_locked = false;
    });
  };
  ...
  $(".signup").each(function () {
    var _signupForm = $(this),
      _signupForm_input = _signupForm.find("input"),
      _signupForm_button = _signupForm.find(".button"),
      placeholder = _signupForm_input.attr("placeholder");
    _signupForm_button.on("click", function (event) {
      event.preventDefault();
      event.stopPropagation();
      _signupForm.submit();
    });
    _signupForm.on("submit", function (event) {
      var v;
      event.preventDefault();
      event.stopPropagation();
      if (_signupForm_input.hasClass("error")) return;
      v = $.trim(_signupForm_input.val());
      if (
        v == "" ||
        !v.match(/^([a-zA-Z0-9\_\-\.\+]+)@([a-zA-Z0-9\-\.]+)\.([a-zA-Z]+)$/)
      ) {
        _signupForm_input.addClass("error");
        window.setTimeout(function () {
          _signupForm_input.removeClass("error");
        }, 1e3);
        return;
      }
      $(":focus").blur();
      _signupForm_input.attr("readonly", "readonly");
      _signupForm.addClass("submitting");
      window.setTimeout(function () {
        $.ajax("/subscribe-responder", {
          method: "POST",
          processData: false,
          contentType: false,
          data: new FormData(_signupForm[0]),
          success: function (x) {
            var delay;
            if (x.result) {
              delay = 3e3;
              _signupForm.addClass("done");
              _signupForm_input.attr(
                "placeholder",
            ...
            window.setTimeout(function () {
              _signupForm_input.attr("placeholder", placeholder);
              _signupForm_input.removeAttr("readonly");
              _signupForm.removeClass("submitting");
              _signupForm.removeClass("done");
            }, delay);
          },
        });
      }, 2e3);
